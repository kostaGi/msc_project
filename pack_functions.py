...
def pack_sig(c_tilde, z, h):
    return c_tilde + bit_pack_z_vector(z) + pack_h(h)

def bit_pack_z_vector(vector):
    return b"".join(bit_pack_z(poly) for poly in vector)

def bit_pack_z(coeffs):
    altered_coeffs = [sub_mod_q(gamma1, c) for c in coeffs]
    ...
    assert gamma1 == (
        1 << 19
    ), f"Expected gamma1 to be either 2^17 or 2^19, got: {gamma1 = }"
...
def unpack_sig(sig_bytes):
    ...
    z = bit_unpack_z_vector(z_bytes, l, 1)
    h = unpack_h(h_bytes)
    return c_tilde, z, tuple(h)

def bit_unpack_z_vector(input_bytes, m, n):
    # Level 2 parameter set
    if gamma1 == (1 << 17):
        packed_len = 576
    # Level 3 and 5 parameter set
        ...
        raise ValueError("Expected gamma1 to be either 2^17 or 2^19")
    ...
    matrix = [
        [bit_unpack_z(poly_bytes[n * i + j]) for j in range(n)] for i in range(m)
    ]
    matrix = [x[0] for x in matrix]
    return matrix

def bit_unpack_z(input_bytes):
    # Level 2 parameter set
    if gamma1 == (1 << 17):
    ...
    coefficients = [gamma1 - c for c in altered_coeffs]
    return coefficients
...
def bit_pack_w(w):
    ...
    assert (
        gamma2 == 261888
    ), f"Expected gamma_2 to be either (q-1)/88 or (q-1)/32, got {gamma2 = }"
