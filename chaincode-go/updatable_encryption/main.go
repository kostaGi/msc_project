...
func main() {
	...
	msg1_mul := sample_random_vector(C.nk*3, 2)
	size := C.nk*2 + 3
	...
	Hmu, b, err2 := Encrypt_multiple(A0, A1, A2, encode(msg1_mul), nil, size)
	...
	msg2, err3 := Decrypt_multiple(R1, Hmu, b, A0, A1, A2, size)
	...
	if !reflect.DeepEqual(msg1_mul[:size], msg2) {
